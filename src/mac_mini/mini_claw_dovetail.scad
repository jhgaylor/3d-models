...
// Four fingers wrap ~15mm down each side. The +X arm can carry a trapezoidal
// dovetail tongue integrated into the plate; the -X arm can carry a matching
// socket cut through a plate extension. Two prints chain end-to-end by
// dropping the second print straight down — its socket envelops the first
...
module claw_outline_2d() {
    difference() {
        union() {
            ...
            if (has_socket)
                // arm-width slab extending past the -X finger; cavity cut below
